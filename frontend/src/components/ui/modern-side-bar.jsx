...
// Navigation items matching project structure
const navigationItems = [
    { id: "dashboard", name: "Dashboard", icon: Home, href: "/dashboard" },
    { id: "academic", name: "Académico", icon: BookOpen, href: "/academic" },
    { id: "financial", name: "Financeiro", icon: CreditCard, href: "/financial" },
    { id: "documents", name: "Documentos", icon: FileText, href: "/documents", badge: "3" },
    { id: "account", name: "Conta", icon: User, href: "/account" },
];

export function Sidebar({ className = "", onLogout, userStatus, children }) {
    const [isOpen, setIsOpen] = useState(false);
    const [isCollapsed, setIsCollapsed] = useState(false);
    const location = useLocation();

    const activeItem = navigationItems.find(item => item.href === location.pathname)?.id || "dashboard";
    ...
    return (
        ...
        `}
            >
                ...
                <div className="flex items-center justify-between p-5 border-b border-slate-200 bg-slate-50/60 h-20">
                    {!isCollapsed && (
                        <div className="flex items-center space-x-2.5 overflow-hidden">
                            <div className="w-9 h-9 min-w-[36px] bg-blue-600 rounded-lg flex items-center justify-center shadow-sm">
                                <span className="text-white font-bold text-base">Z</span>
                            </div>
                            <div className="flex flex-col min-w-0">
                                <span className="font-semibold text-slate-800 text-base truncate">Zedeck's Training</span>
                                <span className="text-xs text-slate-500 truncate">Student Portal</span>
                            </div>
                        </div>
                    )}
                    ...
                    <button
                        ...
                        className="hidden md:flex p-1.5 rounded-md hover:bg-slate-100 transition-all duration-200 absolute right-3"
                        style={{ right: isCollapsed ? 'auto' : '12px', left: isCollapsed ? '50%' : 'auto', transform: isCollapsed ? 'translateX(-50%) translateY(40px)' : 'none' }}
                ...
                <nav className="flex-1 px-3 py-2 overflow-y-auto custom-scrollbar">
                    <ul className="space-y-0.5">
                        {navigationItems.map((item) => {
                            ...
                            const isBlocked = item.href === '/academic' && userStatus === 'blocked';
                        ...
                        `}
                                        ...
                                        {!isCollapsed && (
                                            <div className="flex items-center justify-between w-full">
                                                <span className={`text-sm ${activeItem === item.id ? "font-medium" : "font-normal"}`}>{item.name}</span>
                                                {item.badge && (
                                                    <span className={`
                        ...
                        {!isCollapsed ? (
                            <div className="flex items-center px-3 py-2 rounded-md bg-white hover:bg-slate-50 transition-colors duration-200">
                                <div className="w-8 h-8 bg-slate-200 rounded-full flex items-center justify-center">
                                    <span className="text-slate-700 font-medium text-sm">ST</span>
                                </div>
                                <div className="flex-1 min-w-0 ml-2.5">
                                    <p className="text-sm font-medium text-slate-800 truncate">Student User</p>
            ...
            {/* Main Content Wrapper (Sibling to Sidebar) */}
            <div
                className={`transition-all duration-300 ease-in-out min-h-screen ${isCollapsed ? 'md:ml-20' : 'md:ml-72'}`}
            >
                {children}
            </div>
        </>
